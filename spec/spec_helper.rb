...
def input_id(attr,no); field_id(attr,no,"input") end
def select_id(attr,no); field_id(attr,no,"select") end

# Procmail

def fill_in_a_rule(section,part,substance,no)
  select section, :from => select_id("section", no)
  select part, :from => select_id("part", no)
  fill_in input_id("substance", no), :with => substance 
end
...
def fill_in_an_action(operation,destination,no)
  select operation, :from => select_id("operation", no)
  fill_in input_id("destination", no), :with => destination
end
